...
parser.add_argument('--model_path', type =str, help='dir to load pretrained model', default= 'model_best.pth')   # 
parser.add_argument('--Lambda1', type=float, help='hyper-parameter lambda', default=0.05) 
parser.add_argument('--Lambda2', type=float, help='hyper-parameter lambda', default=0.2)
parser.add_argument('--temperature', type=float, help='temperature parameter in computing probability with prototypes', default=0.5)
...
def compu_proto(feature_bank, feature_labels, args):
    ...
    kmeans = KMeans(n_clusters=args.num_classes, init='random', algorithm='lloyd', random_state = args.seed)
    kmeans.fit(features_np)
    cluster_labels = kmeans.labels_      
    prototypes = kmeans.cluster_centers_
    sample_prototypes = prototypes[cluster_labels]

    map_cluster_labels = np.zeros(args.num_classes, dtype=int)

    for i in range(args.num_classes):
        # 找到属于 cluster i 的样本的标签
        cluster_labels_i = labels_np[np.where(cluster_labels == i)]
        # 找到出现频率最高的标签
        most_common_label = Counter(cluster_labels_i).most_common(1)[0][0]
        map_cluster_labels[i] = most_common_label
    
    origin_label_prototypes = np.zeros_like(prototypes)
    for i in range(args.num_classes):
            origin_label_prototypes[map_cluster_labels[i]] = prototypes[i]

    return sample_prototypes, prototypes, origin_label_prototypes



def compu_supervised_proto(feat1, labels, args):

    # 初始化一个用于存储每一类特征中心的张量
    prototypes = torch.zeros(args.num_classes, feat1.size(1)).to(feat1.device)
    
    # 遍历所有类别
    for cls in range(args.num_classes):
        # 获取当前类别的所有样本的特征
        cls_feat = feat1[labels == cls]  
        # 如果当前类别没有样本，特征中心保持为0
        if cls_feat.size(0) == 0:
            continue
        # 计算当前类别的特征中心（均值）
        prototypes[cls] = cls_feat.mean(dim=0)
...
# Training code
def train(train_loader, epoch, model1, optimizer1,  criterion, sample_prototypes, prototypes, origin_label_prototypes, args):
    ...
    for i, (data, labels, indexes) in enumerate(train_loader):
        ...
        C = sample_prototypes[indexes.cpu().numpy()]
        C = torch.tensor(C).cuda()
        loss_ind = torch.norm((feat1-C), p=2)

        origin_label_prototypes = torch.tensor(origin_label_prototypes, dtype=torch.float32)
        origin_label_prototypes = origin_label_prototypes.cuda()

        supervised_prototypes = compu_supervised_proto(feat1, labels, args)


        # Prevent data overflow
        # if epoch == 0 and i==0:
        #     contain1 = torch.sum(supervised_prototypes * origin_label_prototypes, dim=1)
        #     contain2 = torch.matmul(supervised_prototypes, origin_label_prototypes.T).sum(dim=1)
        #     normalize_flag = torch.max(contain1).item() > 1 or torch.max(contain2).item() > 1
        #     print(normalize_flag)
        # if normalize_flag:  
        #     supervised_prototypes = F.normalize(supervised_prototypes, p=2, dim=1)


        numerator = torch.exp(torch.sum(supervised_prototypes * origin_label_prototypes, dim=1) / args.temperature)
        denominator = torch.exp(torch.matmul(supervised_prototypes, origin_label_prototypes.T) / args.temperature).sum(dim=1)

        similarity_matrix = torch.matmul(supervised_prototypes, supervised_prototypes.T)
        similarity_matrix.fill_diagonal_(0)
        sum_values = torch.sum(torch.exp(similarity_matrix / args.temperature), dim=1)

        loss_clus = -torch.log(numerator / denominator).mean() + sum_values.mean()

        loss = loss + args.Lambda1*loss_ind + args.Lambda2*loss_clus
...
def main(args):
    ...
    feature_bank, feature_labels = feature_extractor(args.model_path, clf1, feat_loader)
    sample_prototypes, prototypes, origin_label_prototypes = compu_proto(feature_bank, feature_labels, args)
    ...
    for epoch in range(0, args.n_epoch):
        ...
        train_acc = train(train_loader, epoch, clf1, optimizer1,  nn.CrossEntropyLoss(), sample_prototypes, prototypes, origin_label_prototypes, args)
